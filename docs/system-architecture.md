...
- **Agreement** - org-scoped public signing record for NDA, Engagement Letter, Service Agreement, Custom, and Consent 7216 flows. Statuses are `DRAFT`, `SENT`, `SIGNED`, `EXPIRED`, and `VOIDED`. Public tokens stay redacted from list responses; only explicit create/send/resend responses return links. Revoked agreements keep history through `voidedAt`, `voidedByUserId`, and bounded `voidReason`, with nullable Staff relation on delete.
