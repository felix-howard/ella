  ...
  useTranslation: () => ({
    t: (key: string, options?: Record<string, unknown> | string) => {
      ...
      if (key === 'team.removeAccessWarningManagedClients') {
        return `${options?.count} managed clients will be unassigned from this member.`
      }
...
describe('RemoveMemberAccessDialog', () => {
  it('renders Clerk access and managed client warnings before confirmation', () => {
    ...
    expect(markup).toContain('The Staff record and audit history remain.')
    expect(markup).toContain('3 managed clients will be unassigned from this member.')
  })
