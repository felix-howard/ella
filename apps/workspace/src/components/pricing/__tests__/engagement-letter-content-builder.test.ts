...
describe('buildCalculatorEngagementLetterHtml', () => {
  it('renders a basic payroll engagement letter with setup and monthly totals', () => {
    ...
    expect(html).toContain('monthly bookkeeping services strictly as described below')
    expect(html).toContain('<p><strong>A. Monthly Bookkeeping</strong></p>')
    expect(html).not.toContain(['monthly bookkeeping and', 'com' + 'pliance services'].join(' '))
    expect(html).not.toContain(['Monthly Bookkeeping and', 'Com' + 'pliance'].join(' '))
    expect(html.toLowerCase()).not.toContain('com' + 'pliance')
