  ...
  return (
    <div className="bg-card rounded-xl border p-4">
      ...
      <p className="text-sm text-muted-foreground mb-3">
        Các tài liệu này trùng với tài liệu đã tải lên. Bạn có thể xóa hoặc phân loại riêng.
      </p>
...
const DuplicateDocItem = memo(function DuplicateDocItem({
  ...
  const handleClassify = async () => {
    ...
    try {
      ...
      toast.success('Đang phân loại tài liệu...')
      onRefresh()
    } catch (error) {
      console.error('[DuplicateDocsCard] Classify failed:', error)
      toast.error('Không thể phân loại tài liệu')
    } finally {
