...
- The calculator can prepare Engagement Letters directly from the current setup/monthly quote. Workspace searches org-scoped clients/leads, snapshots generated HTML into the existing Agreement content editor, defaults initial payment off, uses 30-day expiry, and sends through the normal client/lead Agreement APIs. The generated copy includes manually editable yearly pre-pay wording but does not auto-import a yearly Custom Link amount.
...
**Calculator Engagement Letters:** Workspace `/pricing-calculator` has a fast path for `ENGAGEMENT_LETTER` agreements. It does not create a separate agreement type or backend send API; it builds editable HTML from the current calculator setup/monthly fees, opens the existing content editor, keeps initial payment off by default, and submits through the normal client/lead Agreement create/preview/send flow. Yearly business tax pre-pay copy is manual/editable because yearly payment collection is owned by Custom Link for now.
