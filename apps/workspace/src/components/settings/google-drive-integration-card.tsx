...
export function GoogleDriveIntegrationCard() {
  ...
  const [adminGroupEmail, setAdminGroupEmail] = useState('')
  ...
  useEffect(() => {
    setRootFolderId(data?.connection?.rootFolderId ?? '')
    setAdminGroupEmail(data?.connection?.adminGroupEmail ?? '')
  }, [data?.connection])
  ...
  const canSave = Boolean(data?.connection && rootFolderId.trim())
    ...
    <section className="rounded-xl bg-card shadow-sm" aria-labelledby="google-drive-settings-title">
      ...
      <div className="space-y-4 p-6">
        {!canManageOrganizationSettings ? <p className="text-sm text-muted-foreground">{t('googleDrive.adminOnlyDescription')}</p> : isLoading ? <div className="flex h-16 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" aria-label={t('googleDrive.loading')} /></div> : isError ? <p className="rounded-lg bg-error-light p-3 text-sm text-error" role="alert">{t('googleDrive.loadError')}</p> : <>
          {!data?.isConfigured && <p className="rounded-lg bg-warning-light p-3 text-sm text-warning-foreground" role="status">{t('googleDrive.credentialsMissing')}</p>}
          ...
          {data?.connection ? <p className="text-sm text-muted-foreground">{t('googleDrive.connectedAs', { email: data.connection.googleAccountEmail })}</p> : <p className="text-sm text-muted-foreground">{t('googleDrive.notConnected')}</p>}
          <div className="flex flex-wrap gap-2">
            {!data?.connection && <Button onClick={startConnection} disabled={!canManageOrganizationSettings || !data?.isConfigured || !rootFolderId.trim() || isPending}>{connectMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" aria-hidden="true" />}{t('googleDrive.connect')}</Button>}
            {data?.connection && <Button onClick={() => saveMutation.mutate()} disabled={!canManageOrganizationSettings || !canSave || isPending}><Save className="mr-2 h-4 w-4" aria-hidden="true" />{saveMutation.isPending ? t('googleDrive.testing') : t('googleDrive.saveAndTest')}</Button>}
