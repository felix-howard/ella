...
- **Client** - organizationId FK, managedById FK (Staff, single manager), firstName, lastName, phone, email, language, profile data, intakeAnswers Json, avatarUrl (optional signed R2 URL), notes (HTML up to 50KB), notesUpdatedAt, source (enum: MANUAL|FORM|GENERIC_FORM|STAFF_FORM|CONVERTED, Phase 02 Intake Form), clientType (enum: INDIVIDUAL|BUSINESS, Phase 01 Entity Separation - default INDIVIDUAL), clientGroupId FK (optional, Phase 01 - for grouping related clients). Phase 02: Added businessType (BusinessType enum, nullable), einEncrypted (nullable), businessAddress, businessCity, businessState, businessZip (nullable). Backward-compat fields for clientType=BUSINESS clients before Business entity creation. Primary business details still on Business model.
