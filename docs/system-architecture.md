...
- `apps/workspace/` - Staff dashboard with team management, multi-manager client UI, a staff-visible read-only Services catalogue backed by the paid-services API, and an admin-only pricing calculator/payment-link flow
...
- `/clients/:id` - Client detail with tabbed sections, including a read-only Services tab that groups the paid-services read model into Past Due, Active, Paid one-time, and Ended/Refunded history. It renders safe source/date/cadence/status only: no amounts, receipts, tokens, raw snapshots, Stripe identifiers, or manual mutations. Admin capabilities expose navigation to Payments/Agreements; other staff see provenance without inaccessible links. Overview includes the admin multi-select manager editor and read-only fallback. Tab layout varies by clientType and persists in the URL query string (Phase 15)
...
- `/quote/:payToken` - Public sent-quote payment page. Calculator agreement auto-send signatures link here from the confirmation screen when the linked quote is activated.
...
- Protected API auth requires an active Clerk organization before any Staff database lookup. After lookup or invite-accept bootstrap, `Staff.organization.clerkOrgId` must match the active Clerk organization before tenant context is attached; missing or mismatched context fails with HTTP 403.
...
- **UI Contract:** Nav items (leads, pricing-calculator, /team) gated by capability flags. `/team` visibility uses `canViewTeam`; Team invite dialog + profile role select offer ADMIN/MANAGER/MEMBER app-level roles for admin mutations. Admin Team UI consumes live reconciliation for Staff active/archived counts, Clerk seats used, Access mismatch badges, and remove-access/Invite-again decisions. Code never checks org:admin literal; instead uses semantic capability flags.
- **Team Membership Contract:** Clerk organization membership is source of truth for login access and billable seats. `Staff` rows preserve app profile/history. Remove access is Clerk-first and fail-closed; direct local unarchive is rejected in favor of Clerk invitation restore. Production cleanup steps live in `docs/team-clerk-membership-runbook.md`.
...
- **Calculator quote draft boundary:** Editable Calculator setups are separate from immutable `PaymentQuote` payment/audit records. The first save is explicitly named, then Calculator edits autosave after one second; Custom Link templates and Engagement Letter drafts retain their own lifecycles. The [Workspace draft controls](../apps/workspace/src/components/pricing/quote-drafts/pricing-quote-draft-panel.tsx) and [autosave owner](../apps/workspace/src/components/pricing/quote-drafts/use-pricing-quote-draft-autosave.ts) define the current UI contract.
- **Draft API and concurrency:** ADMIN-only, organization-scoped CRUD uses `GET`/`POST /billing/quote-drafts` and `GET`/`PATCH`/`DELETE /billing/quote-drafts/:id`. PATCH rename/snapshot writes require `expectedUpdatedAt`; a stale timestamp returns `PRICING_QUOTE_DRAFT_CONFLICT` (409) instead of overwriting newer work. Draft writes have separate rate-limit capacity because frequent autosaves must not consume the stricter capacity reserved for payment creation and quote sending. See the [draft API](../apps/api/src/routes/billing/pricing-quote-draft-routes.ts), [rate-limit owner](../apps/api/src/middleware/rate-limiter.ts), [snapshot validation](../apps/api/src/routes/billing/schemas.ts), and [persistence service](../apps/api/src/services/payments/pricing-quote-draft-service.ts).
- **Draft finalization and rollout:** Calculator checkout/send requests may optionally carry `draftId` and `draftUpdatedAt` as a pair. The timestamp pins final consumption to the version Workspace flushed before the final request, so a concurrent newer save is preserved and reported as `draftConsumed=false` with `draftCleanupStatus=version_conflict`; Workspace does not retry cleanup in that case. A failed final action leaves the draft intact and resumes autosave. If a completed payment action instead reports cleanup failure, Workspace retries only the draft deletion and guards it with the same `draftUpdatedAt`, preventing the retry from deleting a newer version. The [billing finalization routes](../apps/api/src/routes/billing/index.ts), [request contract](../apps/api/src/routes/billing/schemas.ts), [draft persistence service](../apps/api/src/services/payments/pricing-quote-draft-service.ts), and [Workspace final-action owner](../apps/workspace/src/components/pricing/pricing-calculator-page.tsx) are executable authority. Before applying the [additive migration](../packages/db/prisma/migrations/20260908130000_add_pricing_quote_drafts/migration.sql), verify a database backup. Deploy API before Workspace so the paired payload never reaches an API version that rejects `draftUpdatedAt`; for an application rollback, restore Workspace before API or take both out of traffic and restore them together. Leave the additive migration in place, preserve migration history, and roll forward for schema corrections.
...
**Account Executive Agreements (MANAGER Staff Compliance):**
- `GET /account-executive-agreements/status` - Authenticated staff status; `required` is derived from `Staff.role === 'MANAGER'` (no separate flag or admin toggle).
- `POST /account-executive-agreements/accept` - MANAGER-only accept with member PNG signature data URL. Server renders the PDF from the shared agreement content via pdf-lib (word-wrap + pagination), auto-filling company name (organization display name), signer name, and signing date; uploads the immutable signed PDF to R2 and persists SHA-256 + signer snapshot. Member-only signature (no firm counter-signature).
- `GET /account-executive-agreements/acceptance/:staffId` and `GET /account-executive-agreements/download/:acceptanceId` - Owner/admin/manager scoped.
- Content source of truth: `packages/shared` `account-executive-agreement-content` (no static PDF asset); version constant `CURRENT_ACCOUNT_EXECUTIVE_AGREEMENT_VERSION`.
- Storage keys: `account-executive-agreements/{orgId}/{staffId}/{version}/{uuid}.pdf`; unique DB acceptance on `[staffId, version]` makes duplicate signing idempotent.
- Workspace gate order (outer→inner): Terms → Contractor → Account Executive.
...
- `POST /webhooks/stripe` - Public Stripe webhook. Verifies `Stripe-Signature` with the raw request body and `STRIPE_WEBHOOK_SECRET`, then syncs local `PaymentQuote`, `StripeCheckoutSession`, and quote-generated `Payment` lifecycle for checkout completion, async payment, invoice, subscription deletion, and `charge.refunded` events. Completed unpaid ACH checkouts remain `awaiting_payment` / public `processing_bank_payment`. Event cursors reject older state, same-second precedence permits `invoice.paid` recovery but blocks failure regression, stale failure events do not alert, and `subscription_canceled` / canceled quote state remains terminal. Duplicate sessions stay sticky as `duplicate_paid_review`. Settlement and receipt enrichment remain best-effort without weakening durable Payment evidence.
...
- **Quote Payment Provenance:** `Payment.paymentQuoteId` is a nullable, indexed relation to `PaymentQuote` with `onDelete: SetNull`. Future first and recurring settled quote payments persist the id from the already-loaded server quote; public request payloads cannot select it. Deposits and historical payments stay null without backfill. Quote/client/lead tenant mismatches fail closed before insertion.
- **Settlement Amount and Refund Evidence:** First quote payments prefer Stripe Checkout `amount_total`; recurring quote payments prefer invoice `amount_paid`. Frozen quote totals are used only when Stripe omits a valid non-negative integer, and explicit zero recurring settlement does not create a ledger row. Full `charge.refunded` events can transition only one uniquely matched quote-linked `Payment` from `PAID` to `REFUNDED`; partial, duplicate, ambiguous, unrelated, or already-refunded events are no-ops. Refund evidence is first persisted on `StripeWebhookEventLog`. Both refund handling and Payment insertion acquire transaction-scoped PostgreSQL advisory locks for every available Charge and PaymentIntent reference in deterministic, deduplicated order. Advisory-lock statements use `$executeRaw`: PostgreSQL returns `void`, which Prisma `$queryRaw` cannot deserialize and surfaced as `P2010` during the first Custom Link smoke. A refund arriving before its Payment is therefore applied when the late Payment is inserted.
- **Recurring Health Recovery:** A newer or precedence-winning `invoice.paid` restores quote/session health from payment failure. Only invoices whose Stripe `billing_reason` is `subscription_cycle` create an idempotent recurring Payment; missing reasons, `subscription_create`, `subscription_update`, and other invoice reasons create no recurring ledger evidence. A newly recorded recurring Payment sends ADMIN SMS to staff with `notifyOnClientPayment`; route-level webhook event logging suppresses already-processed duplicate deliveries, while retries that find an already-written `PAID` Payment can still retry SMS before the event is marked processed. Older failures and same-second failures after success cannot regress health or emit stale alerts. Fresh recurring failures send ADMIN SMS to staff with either `notifyOnClientPayment` or `notifyOnPaymentFailed`; stale/replayed/canceled failures are suppressed. `customer.subscription.deleted` remains terminal and later invoice events cannot reactivate the canceled subscription, while existing paid/refunded Payment rows remain financial history.
- **Paid Services Projection:** `GET /clients/:clientId/paid-services` uses normal organization/assignment scope and returns 404 for an inaccessible client. Eligible Calculator rows require the exact quote-linked signed `CALCULATOR` Engagement Letter; eligible Custom Links must be sent and client-linked; both require direct quote-linked `PAID`/`REFUNDED` Payment evidence. During duplicate-phone lead conversion, settlement moves only that exact signed Calculator agreement to the resolved client, and retry/self-heal paths are idempotent. Evidence stays bounded and aggregated: Payment rows reduce by quote/type/status with earliest paid time, while checkout history reduces to latest health plus cancellation evidence per quote instead of hydrating raw histories. Candidate quotes are keyset-paged by aggregate first settlement (`MIN(Payment.paidAt)` descending, quote ID ascending as the deterministic tie-breaker); malformed or otherwise invalid projections are skipped until the newest 100 valid groups are returned or candidates are exhausted. A 101st valid group produces `meta { isTruncated: true, limit: 100 }`; Workspace shows a notice that older history remains in Payments. The API no longer disables the full view with 422 and does not construct an unbounded aggregate query. The exact non-financial DTO is response `success`, `data`, `meta { isTruncated, limit }`; group `id`, `source`, `paidAt`, nullable `agreement { id, title, signedAt }`, `items`; and item `id`, `label`, nullable `description`, `category`, `cadence`, `status`. Workspace rejects any extra response/meta/group/agreement/item key, including sensitive drift. Labels, descriptions, and agreement titles remain admin-authored free text and can mention pricing, so operational teams keep them suitable for staff visibility.
- **Paid Services Indexes:** Migration `20260716070806_index_payment_stripe_payment_intent` adds `Payment_stripePaymentIntentId_idx` for refund PaymentIntent lookup. It follows the destructive provenance/service-log migration and additive durable refund-evidence migration; applied migration history remains immutable.
...
- **Portal Quote Pay Page:** `PublicQuoteView.billingInterval` exposes per-line cadence (null = one-time). UI surfaces per-line descriptions, correct "Monthly" / "Yearly" group headers, and /mo vs /yr badges. ACH `processing_bank_payment` renders "Bank payment submitted" with 3-5 business day settlement copy and no public retry/pay CTA. Failed payments remain retryable; canceled-before-payment and subscription-canceled-after-payment have separate copy. Calculator agreement auto-send mode returns the activated quote URL from signing, and the portal confirmation renders an explicit `Continue to payment` action instead of redirecting.
- **Workspace Quote Payment Monitoring:** Admin client Payments tab augments ledger `Payment` rows with sent `PaymentQuote` diagnostics. Staff see labels for bank processing, failed retryable quotes, paid, duplicate-payment review before refunding, and subscription-canceled-after-payment. The response includes lightweight monitoring counts for ACH pending after 5 business days, duplicate review sessions, failed quote payments, and paid quotes whose subscription was later canceled. Agreement payment-portal badges hide copy/send actions while ACH is processing or duplicate review is required. Calculator quote send, custom quote send, and agreement payment-portal activation also reject staff-created payment paths when another quote for the same client is still processing bank payment.
...
- The calculator can prepare Engagement Letters directly from the current setup/monthly quote. Workspace searches org-scoped clients/leads, seeds the shared `AgreementDraftEditor` with `source="CALCULATOR"` and a minimal source snapshot, defaults initial payment off, uses 30-day expiry, and requires a saved draft before preview/send. If the recipient already has a CALCULATOR draft, the modal offers `Resume saved calculator draft` vs `Start from current quote`; manual drafts are ignored. Final send uses the draft-send endpoint so the original creator signer is preserved and the actual sender is recorded. The generated copy includes manually editable yearly pre-pay wording but does not auto-import a yearly Custom Link amount. Calculator-linked agreements now keep a pinned `PaymentQuote` and frozen payment-portal mode metadata; agreement payloads expose only a safe quote summary for display, while checkout tokens and snapshots stay server-only. Backend quote lifecycle uses `agreement_draft` before send, `agreement_pending_signature` while waiting for signature, `agreement_signed_review` for manual review, and `sent` once the public pay link is activated. Signing remains successful if quote activation, review marking, or SMS delivery fails; when activation succeeds, the public confirmation page shows `Continue to payment` even if SMS delivery failed. Staff-review and legacy signatures do not show a payment CTA until staff explicitly sends the portal.
- Shared agreement draft closing uses a continuation-based guard across calculator and Client Agreements editors. Clean drafts close immediately; dirty backdrop/X/Escape/Cancel/Back requests open a portaled save-or-discard modal above the editor layer. New drafts wait for successful creation, saved drafts pause debounce autosave and flush the latest `expectedUpdatedAt`-guarded update, failed/conflicted saves keep the editor open, and discarding runs the original close or Back action exactly once.
...
- Local webhook testing: `pnpm dev` starts Stripe CLI forwarding automatically when Stripe CLI plus `STRIPE_SECRET_KEY` and the matching local `STRIPE_WEBHOOK_SECRET` are available. Bootstrap or refresh the local `whsec_...` with `stripe listen --events checkout.session.completed,checkout.session.async_payment_succeeded,checkout.session.async_payment_failed,invoice.paid,invoice.payment_failed,customer.subscription.deleted,charge.refunded --forward-to localhost:3002/webhooks/stripe`, then copy the printed signing secret into `apps/api/.env`. Do not reuse the live Dashboard webhook secret for local CLI forwarding.
- Stripe ACH Direct Debit requires USD presentment and uses delayed settlement. The direct Stripe test account supports ACH: sessions explicitly requesting `us_bank_account` worked. The missing option was caused by Adaptive Pricing localizing USD Checkout to VND in the current locale, which suppressed USD-only ACH. Quote Checkout therefore disables Adaptive Pricing, while production still keeps `STRIPE_CURRENCY=usd` and enables/verifies ACH separately in every test/live Stripe environment.
- Paid Services release state: Phase 6 and its release-readiness rollout are COMPLETE. Final focused validation passes API 68/68, Workspace 50/50, both package type-checks, and `git diff --check`; final reviewer score is 10/10 with no findings or warnings. Earlier Phase 6 evidence remains preserved: 106 migrations current; targeted API 95/95; targeted Workspace 59/59; full suite 3,866/3,866; lint 0 errors/35 known warnings; root type-check 8/8; build 4/4; post-fix API 65/65; simplifier 27/27; independent focused tests 191/191; and USD hardening 97/97. Authenticated ADMIN test-mode smoke verified signed Calculator and linked Custom Link card settlement once each, Past Due → Active → Ended recurring lifecycle without duplicate service, full-refund Refunded retention, and financial detail retention in admin Payments.
- The final auth warning is resolved by the active-Clerk-org precondition and linked-Staff organization cross-check above. Automated regressions verify both paths: a request with no active organization fails before Staff/client database queries, and a Staff record returned for a different Clerk organization fails before client data access.
- Signed Calculator ACH smoke verified delayed settlement end to end: Checkout displayed US bank account; verified `checkout.session.completed` with `payment_status=unpaid` returned 200 and created no linked Payment; verified `checkout.session.async_payment_succeeded` returned 200 and produced exactly one PAID Payment plus one new Active service group.
- Final manual scope evidence: a legitimate same-org STAFF session authenticated; the client list exposed exactly one assigned client; assigned Services UI/API rendered three groups using only the fixed safe DTO with no structured financial or Stripe keys; the unassigned paid-services API returned uniform 404; and direct unassigned page navigation showed `Client not found` without client data. No separate manual cross-org fixture was exercised; cross-org protection remains covered by the mismatched-organization auth regression.
- Coordinated forward-only rollout: in each environment with unapplied migrations, confirm a verified full backup or explicitly confirm legacy `ClientServiceLog` rows are disposable, then obtain explicit destructive approval. Keep `STRIPE_CURRENCY=usd`; enable/verify ACH; configure all required Checkout/async/invoice/subscription/refund events, including `charge.refunded`; enter maintenance and stop the old API/Workspace because the first feature migration removes the legacy schema while adding provenance; apply the committed migrations in order; deploy API, then Workspace; resume traffic; and smoke newly created eligible quotes with ADMIN plus assigned/unassigned staff access. Historical quotes receive no backfill. Phase 6 completion performed no migration, deployment, or Stripe production operation; these environment actions remain separately authorized.
- Rollback boundary: before the destructive migration, the old deployment can be restored. After application, never deploy the old application or edit/recreate applied migrations. Roll forward with corrective app code and, if needed, a new migration; reserve a verified full database backup for disaster recovery.
...
- `GET /org-settings` - Get org profile, org slug, upload-link automation defaults, calculator agreement payment defaults, default upload-link SMS language, and the firm address/governing-law/contact fields used by NDA setup.
- `PATCH /org-settings` - Update org profile, org slug, `autoSendFormClientUploadLink`, `calculatorAgreementPaymentMode`, `defaultUploadLinkTemplateId`, `defaultUploadLinkLanguage`, and the NDA firm-info fields (admin only).
...
- `PATCH /team/members/:staffId/role` - Update role via Clerk Backend API (admin only). ADMIN demotion uses the same org-scoped admin mutation guard as removal.
- `DELETE /team/members/:staffId` - Remove Clerk org access first, then archive Staff locally. Unknown Clerk failures fail closed and do not mark Staff inactive. Already-missing Clerk membership is idempotent.
- `PATCH /team/members/:staffId/archive` - Backward-compatible alias for the same Clerk-first removal flow.
- `PATCH /team/members/:staffId/unarchive` - Direct local restore rejected; restore access by Clerk invitation so membership remains source of truth.
- `GET /team/reconciliation` - Admin-only live Staff vs Clerk comparison with `seatsUsed`, pending invitation count, and mismatch statuses (`ACTIVE_MATCH`, `ARCHIVED_MATCH`, `ARCHIVED_STILL_IN_CLERK`, `ACTIVE_MISSING_CLERK`, `CLERK_MISSING_STAFF`, `PENDING_INVITATION`).
...
**Workspace PWA Web Push:**
- **Subscription APIs** (`apps/api/src/routes/push`): Protected `/push/vapid-public-key`, `/push/subscriptions`, `/push/current`, `/push/subscribe`, `/push/unsubscribe`, and rate-limited `/push/test` endpoints. Subscriptions are scoped to the authenticated Staff + Organization and an endpoint already owned by another staff member returns `409`.
- **Delivery Service** (`apps/api/src/services/web-push`): `sendWebPushToStaff()` loads enabled subscriptions in 500-row pages, sends with VAPID via `web-push`, and disables expired endpoints on provider `404`/`410`. Delivery is best-effort and never blocks message creation.
- **Inbound Message Fanout**: `notifyClientMessagePushFromConversation()` runs for inbound non-system SMS case messages after existing message persistence/realtime paths. Recipients match client visibility: `ADMIN` and `MANAGER` see org-wide clients; `STAFF` and `CPA` receive only when linked through `ClientManager`; inactive staff and staff without enabled push subscriptions are excluded. Inbound portal-authored case messaging does not currently have a separate creation path, so portal fanout is not claimed until that path exists.
- **Privacy Contract**: Payloads contain only `title: "Ella"`, `body: "New client message"`, `/messages/:caseId`, tag, and timestamp. No body text, client name, phone, email, SSN, signed URL, storage key, or tax data is placed in the push payload or push logs.
- **Workspace Client** (`apps/workspace/src/lib/web-push.ts`, `apps/workspace/public/sw.js`): The Settings Notifications card registers `/sw.js`, handles browser support and iOS standalone-PWA checks, saves/removes the current browser subscription, sends test notifications, and constrains notification clicks to `/` or `/messages/:caseId`.
- **Rollout Requirements**: API requires `WEB_PUSH_VAPID_PUBLIC_KEY`, `WEB_PUSH_VAPID_PRIVATE_KEY`, and `WEB_PUSH_VAPID_SUBJECT`. iOS/iPadOS users must open the HTTPS Workspace from a Home Screen install before enabling notifications.
...
- Legacy `CLIENT_SERVICE_LOG` activity identifiers remain readable for historical audit rows only; no current runtime emitter or `ClientServiceLog` schema model remains.

## Database Schema

**Key Models (Multi-Tenant):**
- **Organization** - Org root with Clerk integration, autoSendFormClientUploadLink (bool, Phase 02 Intake Form - auto-send SMS to staff on form submission), calculatorAgreementPaymentMode (AgreementPaymentPortalMode default AUTO_SEND for calculator agreement rows)
...
- **WebPushSubscription** - org/staff-scoped browser push subscription. Fields: organizationId, staffId, endpoint (unique), p256dh, auth, enabled, userAgent, deviceLabel, lastSeenAt, lastSentAt, failureCount, failedAt, createdAt, updatedAt. Used only for Workspace PWA notifications; invalid provider endpoints are disabled automatically.
...
- **Payment** - deposit and quote settlement row. Stores organization/client/lead/agreement links, nullable indexed `paymentQuoteId`, type/status, actual settled amount/currency, payToken, Stripe session/payment-intent/customer/invoice/charge identifiers, receipt artifacts, card brand/last4, paidAt, and receiptSyncedAt. Future first/recurring quote settlements link the server-loaded quote; deposits and historical payments remain unlinked. Quote deletion sets the relation null. Full refunds update the preserved row to `REFUNDED`; partial refunds do not falsely claim full reversal. Staff serializers expose receipt links only after HTTPS Stripe-host validation.
- **Agreement** - org-scoped public signing record for NDA, Engagement Letter, Service Agreement, Custom, and Consent 7216 flows. Statuses are `DRAFT`, `SENT`, `SIGNED`, `EXPIRED`, and `VOIDED`. Agreements can optionally pin one `PaymentQuote` via `paymentQuoteId` and persist `paymentPortalMode` (`NONE`, `AUTO_SEND`, `STAFF_REVIEW`) for calculator payment-portal state. Public tokens stay redacted from list responses; only explicit create/send/resend responses return links. Revoked agreements keep history through `voidedAt`, `voidedByUserId`, and bounded `voidReason`, with nullable Staff relation on delete. Serialized agreement responses include only a safe linked quote summary (`id`, `status`, `sentAt`, `monthlyTotalCents`, `setupTotalCents`) and omit checkout-sensitive quote fields.
- **PaymentQuote** - checkout quote record for Stripe billing. Stores organization/client/lead links, customerEmail/customerName/businessName, inputSnapshot/resultSnapshot, monthlyTotalCents/setupTotalCents, status, createdByStaffId, related checkout sessions, and future settled `Payment` rows. Agreements can point back to one quote for calculator engagement-letter flows; the quote snapshots remain the source of truth for checkout rebuilds. Agreement-linked draft/review statuses are not payable until activation changes the quote to `sent`.
- **PricingQuoteDraft** - organization-scoped editable Calculator setup. Stores a unique name per organization, canonical `inputSnapshot`, derived monthly/setup totals in cents, and timestamps; organization deletion cascades to its drafts. It has no client, lead, Agreement, PaymentQuote, Checkout Session, or Payment relation, preserving the boundary between recoverable work and final payment/audit records.
...
- **StripeWebhookEventLog** - durable Stripe webhook claim and retry record. Stores event/object identity, processing status, attempts, timestamps, and error summary. Additive migration `20260715152629_record_full_refund_webhook_evidence` adds nullable `stripePaymentIntentId` and `chargeFullyRefunded` plus lookup indexes, allowing an unmatched full refund to be reconciled when its quote-generated Payment arrives later without storing raw Stripe payloads.
...
- **AccountExecutiveAgreementAcceptance** - staffId FK + organizationId FK. Account Executive agreement acceptance required for `role = MANAGER` staff. Member-only signature (no firm counter-signature fields). Fields: version, signedAt, signedPdfR2Key, pdfSha256, signerName, signerEmail, signerIpAddress, signerUserAgent. Unique on `[staffId, version]`.
...
**Draft and link boundary:** Authenticated agreement list APIs return agreement metadata, source snapshots, and staff summaries but redact raw signing `token` and public signing `url`. Explicit create/send/resend flows are the only workspace API paths that return a public signing link. Public `/public/agreements/:token` and `/public/nda/:token` render only active SENT rows, while send/resend/extend lifecycle writes use entity advisory locks plus scoped status guards to avoid mutating DRAFT, SIGNED, VOIDED, or newly superseded NDA agreements. Saved draft sends snapshot the original creator's firm signer profile and record the actual sender separately for audit. Calculator draft/send payloads can carry a `calculatorQuote` snapshot plus optional `paymentPortalMode`; when omitted, the org-level `calculatorAgreementPaymentMode` default applies.

**Revocation lifecycle:** Staff can revoke only unsigned `SENT` or `EXPIRED` agreements through lead/client Agreement endpoints. Revocation updates the row to `VOIDED`, sets `isActive=false`, stores `voidedAt`, `voidedByUserId`, and bounded `voidReason`, and writes a redacted Agreement activity entry. Signed agreements remain immutable, and drafts continue to use the draft discard flow. Public signing links for `VOIDED` rows return a sanitized revoked state and never expose the original document or revocation reason.

**Workspace draft editor:** Manual Agreement wizard flows and pricing-calculator Engagement Letters use the same draft-capable editor. Opening the editor does not create a draft. Staff can send immediately through the existing create/send path, or explicitly `Save draft`; autosave begins only after that first successful save and sends `expectedUpdatedAt` on later updates. Calculator entries are draft-first: the modal only resumes `DRAFT` + `CALCULATOR` rows, offers `Resume saved calculator draft` vs `Start from current quote`, and blocks preview/send until a draft exists. Conflict responses block final send and surface a reload action instead of merging or overwriting remote content.

**Workspace draft cards:** The agreement list now treats saved drafts as resumable cards, sorts draft rows first by latest edit, and limits draft actions to `Resume` and `Discard`. Draft discard requests also carry `expectedUpdatedAt` so stale cards cannot delete newer saves.

**Calculator Engagement Letters:** Workspace `/pricing-calculator` has a fast path for `ENGAGEMENT_LETTER` agreements. It does not create a separate agreement type or backend send API; it builds editable HTML from the current calculator setup/monthly fees, opens the shared draft editor, keeps initial payment off by default, seeds a minimal `sourceSnapshot` (`preparedAt`, recipient id/type, setupTotal, monthlyTotal, tierLabel), and finalizes through the draft-send path once a draft exists. If the signed agreement stays in `STAFF_REVIEW`, the workspace uses `send-payment-portal` on the client/lead agreement route to activate the linked calculator quote after review; the response returns `payUrl` plus SMS status and does not expose the raw `payToken`. Yearly business tax pre-pay copy is manual/editable because yearly payment collection is owned by Custom Link for now.
...
- **Staff Notifications** - ADMIN-only toggles: `notifyOnAgreementSigned` (SMS to admins when client signs), `notifyOnClientPayment` (SMS to admins when client pays, including recurring quote invoices and failed recurring collection), `notifyOnPaymentFailed` (failure-only SMS to admins when recurring quote collection fails)
...
- **Settings → Notifications**: 3 ADMIN-only toggles (`notifyOnAgreementSigned`, `notifyOnClientPayment`, `notifyOnPaymentFailed`), default OFF. PATCH endpoint rejects toggles for non-ADMIN targets (403)
...
**Status:** Automatic Paid Client Services Phase 6 and release-readiness rollout are complete. ADMIN card/ACH/lifecycle and legitimate same-org STAFF assigned/unassigned scope smoke passed. Final validation: API 68/68, Workspace 50/50, API/Workspace package type-checks, `git diff --check`, and final reviewer 10/10 with no findings or warnings. Automated missing/mismatched Clerk organization regressions pass; no separate manual cross-org fixture was exercised. No migration, deployment, or Stripe production operation was performed during completion. English-first i18n enforcement complete. Multi-Tenant architecture with Clerk Webhook Sync Migration complete. Client-Business Entity Separation Phase 06 (Cleanup & Integration Testing) complete. Supabase Realtime Broadcast integrated for 100-500ms message updates. Phase 12 Client Creation Wizard (multi-path) complete. Phase 13 Client Detail Page (type-based tabs) complete. Phase 02 ShareableDocument (multi-section) API + Phase 04 Workspace UI complete: SharedDocsTab + 10 sub-components, useSharedDocs/useSharedDocSignedUrl hooks, legacy DraftReturn components removed. Draft Return tab renamed to Shared Docs tab across UI and i18n keys.
