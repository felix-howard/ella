/**
 * Client-side defence-in-depth sanitizer for NDA HTML.
 * Server already sanitized at write (apps/api/src/lib/nda/sanitize-html.ts);
 * this strips anything that slipped past or was injected by a man-in-the-middle.
 *
 * Whitelist mirrors the backend NDA whitelist (NDA_ALLOWED_TAGS + scheme set
...
export function sanitizeNdaHtmlClient(html: string): string {
