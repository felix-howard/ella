...
- `apps/workspace/` - Staff dashboard with team management, multi-manager client UI, client service ledger, and an admin-only pricing calculator/payment-link flow
...
- `/clients/:id` - Client detail with tabbed sections, including a staff-only Services tab for the internal service ledger. Overview includes the admin multi-select manager editor and read-only fallback. Tab layout varies by clientType and persists in the URL query string (Phase 15)
...
- `POST /webhooks/stripe` - Public Stripe webhook. Verifies `Stripe-Signature` with the raw request body and `STRIPE_WEBHOOK_SECRET`, then syncs local `PaymentQuote` and `StripeCheckoutSession` status for `checkout.session.completed`, `checkout.session.async_payment_succeeded`, `checkout.session.async_payment_failed`, `invoice.paid`, `invoice.payment_failed`, and `customer.subscription.deleted`. Completed unpaid ACH checkouts map to `awaiting_payment` / public `processing_bank_payment`, and the existing local checkout-session quote association wins over Stripe metadata when both exist. Webhook updates persist `lastStripeEventId` and `lastStripeEventAt` on quote/session records, store Stripe customer/invoice/paymentIntent ids on the session mirror, ignore older events, apply same-second status precedence, and keep duplicate sessions sticky as `duplicate_paid_review` for staff review. Paid deposit and recurring quote events also capture receipt facts best-effort; missing Stripe receipt data or retrieval errors do not block settlement.
...
- **ClientServiceLog** - org/client-scoped internal service ledger for Workspace staff. Fields: organizationId, clientId, serviceType, customServiceName, status, taxYear, serviceDate, note, createdById, updatedById, deletedAt, deletedById, createdAt, updatedAt. Staff relations are tenant-scoped through organizationId. Service logs soft-delete through `deletedAt` and never expose Portal routes. ActivityLog entries use `CLIENT_SERVICE_LOG` target metadata without note bodies.
...
- **PaymentQuote** - checkout quote record for Stripe billing. Stores organization/client/lead links, customerEmail/customerName/businessName, inputSnapshot/resultSnapshot, monthlyTotalCents/setupTotalCents, status, createdByStaffId, and related checkout sessions. Agreements can point back to one quote for calculator engagement-letter flows; the quote snapshots remain the source of truth for checkout rebuilds. Agreement-linked draft/review statuses are not payable until activation changes the quote to `sent`.
