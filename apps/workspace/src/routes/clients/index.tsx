  ...
  return (
    <PageContainer>
      ...
      <div className="flex flex-col gap-4 mb-6">
        {/* Row 1: Search + Managed By dropdown */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          {isAdmin && (
            <select
              value={managedById ?? ''}
              onChange={(e) => setManagedById(e.target.value || undefined)}
              className="appearance-none pl-3 pr-8 py-2.5 rounded-full border-none bg-card shadow-sm text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 cursor-pointer"
              aria-label={t('clients.managedBy')}
            >
              <option value="">{t('clients.allMembers')}</option>
              {staffId && <option value={staffId}>{t('clients.me')}</option>}
              {teamMembers
                .filter(m => m.id !== staffId && m.isActive)
                .map(m => (
