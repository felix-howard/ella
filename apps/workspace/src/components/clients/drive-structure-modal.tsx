...
import { FolderPlus, Loader2, Mail, Pencil, Shield } from 'lucide-react'
...
interface DriveStructureModalProps {
  open: boolean
  clientId: string
  isPending: boolean
  onClose: () => void
  onSubmit: (input: ClientDriveStructureCreateInput) => void
  onAddClientEmail?: () => void
}
interface FormState { ssnLast4: string; state: string; businessMode: ClientDriveBusinessMode; businessName: string; sendNotificationEmail: boolean }

function buildInitialForm(options: ClientDriveStructureOptionsDto): FormState {
  return { ssnLast4: '', state: options.defaultState ?? '', businessMode: options.defaultBusinessMode, businessName: options.defaultBusinessName ?? '', sendNotificationEmail: false }
}
...
export function DriveStructureModal({ open, clientId, isPending, onClose, onSubmit, onAddClientEmail }: DriveStructureModalProps) {
  if (!open) return null
  return <DriveStructureModalContent key={clientId} clientId={clientId} isPending={isPending} onClose={onClose} onSubmit={onSubmit} onAddClientEmail={onAddClientEmail} />
}

function DriveStructureModalContent({ clientId, isPending, onClose, onSubmit, onAddClientEmail }: Omit<DriveStructureModalProps, 'open'>) {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!initialForm || !/^\d{4}$/.test(initialForm.ssnLast4) || !/^[A-Za-z]{2}$/.test(initialForm.state.trim()) || !hasClientEmail || (initialForm.businessMode === 'SINGLE_BUSINESS' && !initialForm.businessName.trim())) { setError(t('googleDrive.validationError')); return }
    onSubmit({ ssnLast4: initialForm.ssnLast4, state: initialForm.state.trim().toUpperCase(), businessMode: initialForm.businessMode, businessName: initialForm.businessMode === 'SINGLE_BUSINESS' ? initialForm.businessName.trim() : undefined, clientEmail, sendNotificationEmail: initialForm.sendNotificationEmail })
  }
  const canSubmit = Boolean(initialForm && /^\d{4}$/.test(initialForm.ssnLast4) && /^[A-Za-z]{2}$/.test(initialForm.state.trim()) && hasClientEmail && (initialForm.businessMode === 'MULTI' || initialForm.businessName.trim()))
  return <Modal open onClose={onClose} size="lg" closeOnEscape={!isPending} closeOnOverlayClick={!isPending} showCloseButton={!isPending} aria-labelledby="drive-structure-title" aria-describedby="drive-structure-description"><form onSubmit={submit}><ModalHeader><ModalTitle id="drive-structure-title">{t('googleDrive.modalTitle')}</ModalTitle><ModalDescription id="drive-structure-description">{t('googleDrive.modalDescription')}</ModalDescription></ModalHeader><ModalBody className="max-h-[65vh] space-y-4 overflow-y-auto">
    {isLoading ? <div className="flex h-32 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin" aria-label={t('googleDrive.loading')} /></div> : isError || !initialForm || !options ? <p className="rounded-lg bg-error-light p-3 text-sm text-error" role="alert">{t('googleDrive.optionsError')}</p> : <>
      ...
      <div>
        <p className="mb-1.5 text-sm font-medium">{t('googleDrive.clientDriveAccess')}</p>
        {hasClientEmail ? (
          <div className="flex flex-col gap-3 rounded-lg border border-border bg-card p-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-start gap-2.5">
              <Mail className="mt-0.5 h-4 w-4 flex-shrink-0 text-muted-foreground" aria-hidden="true" />
              <div className="min-w-0">
                <p className="break-words text-sm font-medium text-foreground">{clientEmail}</p>
                <p className="text-xs text-muted-foreground">{t('googleDrive.clientEmailSaved')}</p>
              </div>
            </div>
            {onAddClientEmail && (
              <Button type="button" variant="outline" size="sm" onClick={onAddClientEmail} disabled={isPending} className="w-full sm:w-auto">
                <Pencil className="mr-1.5 h-3.5 w-3.5" aria-hidden="true" />
                {t('googleDrive.changeClientEmail')}
              </Button>
            )}
          </div>
        ) : (
          <div className="rounded-lg border border-warning/30 bg-warning/5 p-3">
            <div className="flex items-start gap-2.5">
              <Mail className="mt-0.5 h-4 w-4 flex-shrink-0 text-warning" aria-hidden="true" />
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-foreground">{t('googleDrive.clientEmailMissing')}</p>
                <p className="mt-1 text-sm text-muted-foreground">{t('googleDrive.clientEmailMissingHelp')}</p>
                {onAddClientEmail && (
                  <Button type="button" variant="outline" size="sm" onClick={onAddClientEmail} disabled={isPending} className="mt-3 w-full sm:w-auto">
                    <Pencil className="mr-1.5 h-3.5 w-3.5" aria-hidden="true" />
                    {t('googleDrive.addClientEmail')}
                  </Button>
                )}
              </div>
            </div>
      ...
      <label className="flex items-center justify-between gap-3 rounded-lg border border-border p-3 text-sm"><span>{t('googleDrive.sendNotification')}</span><Switch checked={hasClientEmail && initialForm.sendNotificationEmail} onCheckedChange={(checked) => update('sendNotificationEmail', checked)} disabled={isPending || !hasClientEmail} /></label>
      <div className="rounded-lg bg-muted/50 p-3 text-sm">
        ...
        <ul className="mt-1 space-y-1 text-muted-foreground">
          ...
          <li>{hasClientEmail ? t('googleDrive.permissionClient', { email: clientEmail }) : t('googleDrive.permissionClientMissing')}</li>
        </ul>
      </div>
